...
export default function LockScreen(props) {
    ...
    return (
        <div id="ubuntu-lock-screen" style={{ zIndex: "100" }} className={(props.isLocked ? " visible translate-y-0 " : " invisible -translate-y-full ") + " absolute outline-none bg-black bg-opacity-90 transform duration-500 select-none top-0 right-0 overflow-hidden m-0 p-0 h-screen w-screen"}>
            ...
            <div className="w-full h-full z-50 overflow-hidden relative flex flex-col justify-center items-center">
                <div className="font-mono tracking-wider" style={{ fontSize: '5rem', color: '#c5c8c6', fontWeight: 300 }}>
                    <Clock onlyTime={true} />
                </div>
                <div className="mt-2 text-sm font-mono tracking-widest" style={{ color: '#7c7c7c' }}>
                    <Clock onlyDay={true} />
                </div>
                <div className="mt-16 text-xs font-mono tracking-wide" style={{ color: '#555' }}>
